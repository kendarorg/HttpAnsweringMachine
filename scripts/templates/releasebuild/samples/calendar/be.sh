...
HAM_JAR=janus-driver-1.1.4.jar
CALENDAR_PATH=$(pwd)
cd $CALENDAR_PATH
# Go to main path
cd ..
...
rm tmp_txt || true
java -cp "be-4.1.3-SNAPSHOT.jar;../janus-driver-1.1.4.jar" org.springframework.boot.loader.JarLauncher --spring.config.location=file://$(pwd)/application.properties &
cd $START_LOCATION
