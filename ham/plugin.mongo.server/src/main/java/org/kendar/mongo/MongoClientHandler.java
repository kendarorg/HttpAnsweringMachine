...
public class MongoClientHandler implements Runnable {
    ...
    private Map<Integer, MsgHandler> msgHandlers;
    private Map<Integer, CompressionHandler> compressionHandlers;

    public MongoClientHandler(Socket client, List<MsgHandler> msgHandlers, List<CompressionHandler> compressionHandlers) {
        this.client = client;
    ...
    private MongoPacket readPacketsFromStream(InputStream inputStream, byte[] headerBytes) throws IOException {
        ...
        try {
            ...
            if (opCode == OpCodes.OP_COMPRESSED) {
                handleOpCompressedCode(requestId, responseTo, packet, remainingBytes, remainingInput);
            } else {
    ...
    private void handleOpCode(int opCode, ByteBufferBsonInput bsonInput, ByteBuf byteBuffer, MongoPacket packet,
                                     int length) {
