...
public class CertitificatesGeneratorTest {
    ...
    public void generateFromScratch() throws Exception {
        ...
        GeneratedCert rootCA = target.createCertificate("CN=do_not_trust_test_certs_root",null,null,extraDomains,true);
        GeneratedCert issuer = target.createCertificate("CN=do_not_trust_test_certs_issuer",null,rootCA,extraDomains,true);
        GeneratedCert domain = target.createCertificate("CN=local.gamlor.info","local.gamlor.info", issuer,extraDomains,false);
        GeneratedCert otherD = target.createCertificate("CN=other.gamlor.info","other.gamlor.info", issuer,extraDomains,false);
    }
    @Test
    ...
    public void generateFromRoot() throws Exception {
        ...
        var root = target.loadRootCertificate("certificates/ca.der","certificates/ca.key");
        var extraDomains = new ArrayList<String>();
        GeneratedCert domain = target.createCertificate("CN=local.gamlor.info","local.gamlor.info", root,extraDomains,false);
        //GeneratedCert otherD = target.createCertificate("other.gamlor.info","other.gamlor.info", root,extraDomains,false);
    ...
    public void generateMultiple() throws Exception {
        ...
        extraDomains.add("kendar.org");
        GeneratedCert domain = target.createCertificate("CN=kendar.org,O=Local Development, C=US",null, root,extraDomains,false);
        var encodedBytes = domain.certificate.getEncoded();
    ...
    public void generateMultiple2() throws Exception {
        ...
        extraDomains.add("kendar.org");
        extraDomains.add("*.kendar.org");
        GeneratedCert domain = target.createCertificate("C=US,O=Local Development,CN=kendar.org",null, root,extraDomains,false);
        var encodedBytes = domain.certificate.getEncoded();
