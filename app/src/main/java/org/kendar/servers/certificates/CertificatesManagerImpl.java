...
public class CertificatesManagerImpl implements CertificatesManager{
    ...
    public GeneratedCert createCertificate(String cnName, String rootDomain, GeneratedCert issuer,
        ...
        if(!isCA){
           byte[] extvalue = issuer.certificate.getExtensionValue(Extension.authorityKeyIdentifier.getId());
            if(extvalue!=null) {
                byte[] filteredByteArray = Arrays.copyOfRange(extvalue, extvalue.length - 20, extvalue.length);

                AuthorityKeyIdentifier authorityKeyIdentifier = new AuthorityKeyIdentifier(filteredByteArray);
